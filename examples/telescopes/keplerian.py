...
def objective_focus_x() -> float:
    ...
    tracer = SequentialTracer(OpticalSystem(rows, object_space=AIR))
...
def build_scene(gap: float, intermediate_image_x: float) -> Scene:
    ...
    scene.add(
        MarkerItem(
            points=np.array([[intermediate_image_x, 0.0]]), color=(0.3, 1.0, 0.4, 1.0), size=8.0
        )
    )
    return scene
...
def main() -> None:
    ...
    print(
        f"  real intermediate image near z={intermediate_image_x:.2f} mm "

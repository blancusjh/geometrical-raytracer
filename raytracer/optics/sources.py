"""Ray sources (emitters) for 2-D scenes.

A ``Source`` is the other physical-object abstraction a propagation
algorithm needs alongside ``Ray`` and ``Surface``: given its own geometry
(origin, aperture, ...) it produces the seed rays (:class:`RaySeed`) a
tracer starts propagating from. It does not know how to propagate or
collide those rays -- that is the propagation algorithm's job.
"""
...
class Source:
    """Base class for 2-D ray sources (emitters)."""
...
class ImageSource(Source):
    ...
    system and histogramming a :class:`~raytracer.optics.instruments.Screen`
    ...
    @property
    def samples(self) -> int:  # total emitted rays (Source contract)

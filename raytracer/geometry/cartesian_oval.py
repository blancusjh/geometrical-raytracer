...
``zi`` (index ``ni``). ``raytracer/geometry/ovoid2d.py`` traces this surface
implicitly via ``F(z, r) = 0``. This module instead provides the equivalent
...
- :func:`cartesian_oval_sag_and_slope` — the usual optics ``sag(h) -> z``
  ...
  3-D sequential tracer, which expects ``profile.sag_and_slope(h)`` exactly
  like :class:`raytracer.geometry.sag.AsphereProfile` (duck-typed, see
  ``raytracer/sequential/trace.py``).
...
class CartesianOvalProfile:
    ...
    Duck-typed to the same contract as :class:`raytracer.geometry.sag.AsphereProfile`
    ...
    intersection in ``raytracer/sequential/trace.py`` only ever calls
    ``profile.sag_and_slope(h)``.

    Unlike the 2-D ``CartesianOvoid2D`` (implicit, solved by bisection on
